				...
				<div class="box box-primary" style="padding-top:20px;">
					...
					<div class="col-md-12" style="margin-bottom:10px;">
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control" placeholder="TAL ID" id="mn_id" name="mn_id"/> 
						</div>
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control" placeholder="Events Title" id="mn_title" name="mn_title"/> 
						</div>
						<div class="col-md-2 text_box_padding">
						...
						<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
							<thead>
								<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
									...
									<th style="text-align:left">TAL ID</th> 
...
<script>
	...
	function loadData(page,fields,ord_sort){
		...
		requ = $.ajax({
			type:"POST",
			data:"mn_title="+mn_title+"&mn_id="+mn_id+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort,
			url:"<?php echo $admin_url?>ad_data.php?call=admin_events_list",
			beforeSend: function(){
				$(".response_gif").show();
			},
	...
	function editbtn(id){ 
		$(".response_gif").show();
		window.location = "<?php echo $admin_url ?>events/"+btoa(id);
	}

	function deletebtn(id,eid){
		if(requ && requ.readyState != 4){
			requ.abort();
		}
		requ = $.ajax({
			type:"POST",
			data:{id:id, eid:eid},
			url:"<?php echo $admin_url?>ad_data.php?call=delete_events",
			beforeSend: function(){
				$(".response_gif").show();
			},
