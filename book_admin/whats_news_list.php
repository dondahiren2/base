...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo $admin_url?>whats_news_list">Whats News List</a>
		</li>
	</ul>	
</div>
				...
				<div class="box box-primary" style="padding-top:20px;">
					...
					<div class="col-md-12" style="margin-bottom:10px;">
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control" placeholder="TAL UNIQUE ID" id="mn_tal_id" name="mn_tal_id"/> 
						</div>
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control" placeholder="Whats News Title" id="mn_title" name="mn_title"/> 
						</div>
						<div class="col-md-2 text_box_padding">
						...
						<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
							<thead>
								<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
									...
									<th style="text-align:left">TAL UNIQUE ID</th>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#btn_reset").click(function () {
			...
			document.getElementById("mn_tal_id").value = '';  
			loadData(page,fields,ord_sort);
		});
	...
	function loadData(page,fields,ord_sort){
		...
		var mn_tal_id = document.getElementById("mn_tal_id").value; 
		...
		requ = $.ajax({
			type:"POST",
			data:"mn_title="+mn_title+"&mn_tal_id="+mn_tal_id+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort+"&mn_type=whats_news",
			url:"<?php echo $admin_url?>ad_data.php?call=admin_whats_news_list",
			beforeSend: function(){
				$(".response_gif").show();
			},
	...
	function deletebtn(id, bp_tal_id){
		if(requ && requ.readyState != 4){
			requ.abort();
		}
		requ = $.ajax({
			type:"POST",
			data:"id="+id+"&bp_tal_id="+bp_tal_id,
