		...
		<div class="row"> 
			<div class="col-md-12">
				<div class="box box-primary" style="padding-top:20px;">
					...
					<div class="col-md-12" style="padding-top:0px;margin-bottom:10px;">
						<div class="col-md-2"><input type="text" class="form-control saerchbtn" placeholder="Name" id="m_nm" name="m_nm"/></div>
					</div>
					...
					<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
						<thead>
							<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
								...
								<th style="width:100px;text-align:left">Medium Name
									<span style="width: 10px; float: right;">
									<i class="fa fa-caret-up" id="medium_name" data-id="asc" aria-hidden="true" style="float: right; position: absolute;cursor: pointer;"></i>
									<i class="fa fa-caret-down" id="medium_name" data-id="desc"  aria-hidden="true" style="float: right; position: absolute; margin-top: 10px;cursor: pointer;"></i>
									</span>
								</th>			
								<th style="width:70px">Action</th>
							</tr>
						</thead> 
						<tbody id="medum_list"></tbody> 
					</table>
	...
	$("#deleteall").click(function() { 
		...
		if (r == true) {
			...
			requ = $.ajax({
				type:"post",
				data:"id="+id, 
				url:"ad_data.php?call=delete_medium",
...
loadData(page,fields,ord_sort);
	function loadData(page,fields,ord_sort){
		var m_nm = document.getElementById("m_nm").value;
		...
		requ = $.ajax({
			type:"POST",
			data:"m_nm="+m_nm+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort,
			url:"ad_data.php?call=medium_list",
			success: function(msg){
				//alert(msg);
				var msga = msg.split("pagination_part");
				$("#medum_list").html(msga[0]);
...
function editbtn(id){ 
	window.location = "medium.php?id="+id;
}
