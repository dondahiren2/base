				...
				<div class="box box-primary" style="padding-top:20px;">
					<div class="col-md-12" style="padding-top:0px;margin-bottom:10px;">
						...
						<div style="float:right">
							<input type="submit" class="btn btn-success"  id="create_master_category" value="Create Form" name="create_prime_category" />
						</div>
					</div>
					<div class="col-md-12" style="margin-bottom:10px;">
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control saerchbtn search_master_name" placeholder="Select Department" id="s_m_name" name="s_m_name"/>
						...
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control saerchbtn" placeholder="Form Name" id="form_name" name="form_name"/>
						</div>
						<div class="col-md-2 text_box_padding">
						...
						<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
							<thead>
								<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
									...
									<th style="width:150px;text-align:left">Form Name
										<span style="width: 10px; float: right;">
											<i class="fa fa-caret-up" id="cf_p_id" data-id="asc" aria-hidden="true" style="float: right; position: absolute;cursor: pointer;"></i>
											<i class="fa fa-caret-down" id="cf_p_id" data-id="desc"  aria-hidden="true" style="float: right; position: absolute; margin-top: 10px;cursor: pointer;"></i>
										</span>
									</th>
									<th style="width:150px;text-align:left">Display
										<span style="width: 10px; float: right;">
											<i class="fa fa-caret-up" id="cf_status" data-id="asc" aria-hidden="true" style="float: right; position: absolute;cursor: pointer;"></i>
											<i class="fa fa-caret-down" id="cf_status" data-id="desc"  aria-hidden="true" style="float: right; position: absolute; margin-top: 10px;cursor: pointer;"></i>
										</span>
									</th>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#create_master_category").click(function() { 
			window.location = "<?php echo $admin_url ?>form_create";
		});
	...
	function loadData(page,fields,ord_sort){
		...
		requ = $.ajax({
			type:"POST",
			data:"page="+page+"&fields="+fields+"&ord_sort="+ord_sort+"&form_name="+form_name+"&fp_m_id="+fp_m_id,
			url:"<?php echo $admin_url?>ad_data.php?call=create_form_page_list",
			beforeSend: function(){
				$(".response_gif").show();
			},
	...
	function editbtn(id){ 
		$(".response_gif").show();
		window.location = "<?php echo $admin_url ?>form_create/"+btoa(id);
	}
	...
	function deletedbtn(id){
		...
		requ = $.ajax({
			type:"POST",
			data:"id="+id,
			url:"<?php echo $admin_url?>ad_data.php?call=form_create_delete",
			beforeSend: function(){
				$(".response_gif").show();
			},
