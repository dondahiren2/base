...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<li>
			<a href="<?php echo $admin_url?>micro-website-permission">Micro Website Permission</a>
		</li>
	</ul>	
</div>
				...
				<div class="box box-primary" style="padding-top:20px;">
					<div class="col-md-12" style="padding-top:0px;margin-bottom:10px;">
						...
						<div style="float:right">
							<input type="submit" class="btn btn-success"  id="create_master_category" value="Create Micro Website Permission" name="create_prime_category" />
						</div>
					</div>
					<div class="col-md-12" style="margin-bottom:10px;">
						...
						<div class="col-md-2 text_box_padding">
							<input type="text" class="form-control saerchbtn" placeholder="Website Name" id="s_name" name="s_name"/>
						</div>
						<div class="col-md-2 text_box_padding">
						...
						<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
							<thead>
								<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
									...
									<th style="width:150px;text-align:left">Website Name</th>
									<th style="width:150px;text-align:left">URL</th>		
...
<script>
	...
	$(document).ready(function() {
		...
		$("#create_master_category").click(function() { 
			window.location = "<?php echo $admin_url ?>micro_website_permission_add";
		});
	...
	function loadData(page,fields,ord_sort){ 
		...
		requ = $.ajax({
			type:"POST",
			data:"s_m_name="+s_m_name+"&s_p_name="+s_p_name+"&s_name="+s_name+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort+"&display=1",
			url:"<?php echo $admin_url?>ad_data.php?call=micro_website_permission_list",
			beforeSend: function(){
				$(".response_gif").show();
			},
	...
	function editbtn(id){ 
		$(".response_gif").show();
		window.location = "<?php echo $admin_url ?>micro_website_permission_add/"+btoa(id);
	}
	...
	function deletedbtn(id){
		...
		requ = $.ajax({
			type:"POST",
			data:"id="+id,
			url:"<?php echo $admin_url?>ad_data.php?call=delete_micro_website_permission",
			beforeSend: function(){
				$(".response_gif").show();
			},
