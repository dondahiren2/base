						...
						<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
							<thead>
								<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
									...
									<th style="width:60px">Sub Code</th>
...
<script>
	...
	$(document).ready(function() {
		...
		$("#create_master_category").click(function() { 
			window.location = "<?php echo $admin_url ?>sub_cate/<?php echo $title?>/<?php echo $title_ids ?>/create";
		});
	...
	function loadData(page,fields,ord_sort){
		...
		requ = $.ajax({
			type:"POST",
			data:"s_cc_name="+s_cc_name+"&s_m_name="+s_m_name+"&s_p_name="+s_p_name+"&s_name="+s_name+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort+"&display=0",
	...
	function editbtn(id){ 
		$(".response_gif").show();
		window.location = "<?php echo $admin_url ?>sub_cate/<?php echo $title?>/<?php echo $title_ids ?>/"+btoa(id);
	}
	...
	function deletedbtn(id){
		...
		requ = $.ajax({
			type:"POST",
			data:"id="+id,
			url:"<?php echo $admin_url?>ad_data.php?call=delete_sub",
			beforeSend: function(){
				$(".response_gif").show();
			},
