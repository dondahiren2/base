...
<link rel="stylesheet" href="https://foliotek.github.io/Croppie/croppie.css">
	<div class="page-bar">
		<ul class="page-breadcrumb">
			...
			<li>
				<a href="#">Create New Team Members</a>
			</li>
		</ul>
	</div>
	<div class="row">
...
$(document).ready(function() {
	...
	$('.upload-result').on('click', function (ev) {
		$uploadCrop.croppie('result', {
			...
			var bp_crop_name = bp_fname+' '+bp_lname;
			$.ajax({
				url: "<?php echo $site_url?>ajaxpro",
				type: "POST",
				data: {"image":resp, "type":"TAL", "parts":"Team", "unique_id": unique_id, "tal_id": tal_id, "names": bp_crop_name },
	...
	$("#submit_ngo").click(function (){ 
		...
		var bp_designation = document.getElementById('bp_designation').value;
		var bp_user_role = document.getElementById('bp_user_role').value;
		var bp_status = document.getElementById('bp_status').value;
		var bp_show_on_webiste = document.getElementById('bp_show_on_webiste').value;
		var bp_remark = document.getElementById('bp_remark').value;
		...
		if(bp_fname == ''){
			...
			requ = $.ajax({
				type:"POST",
				data:"id=<?php echo $id?>&idArray="+idArray+"&valueArray="+valueArray+"&unique_id="+unique_id+"&con_id="+con_id+"&con_val="+con_val+"&bcd_data_type="+bcd_data_type+"&bank_id="+bank_id+"&bank_val="+bank_val+"&bank_data_type="+bank_data_type+"&kyc_data_type="+kyc_data_type+"&kyc_val="+kyc_val+"&kyc_id="+kyc_id+"&ref_val="+ref_val+"&ref_id="+ref_id+"&ref_data_type="+ref_data_type+"&bp_designation="+bp_designation+"&bp_user_role="+bp_user_role+"&bp_status="+bp_status+"&bp_show_on_webiste="+bp_show_on_webiste+"&bp_remark="+bp_remark+"&bp_sub_user_type=<?php echo $team_user_type?>&insert_id=<?php echo $insert_id?>&department_code=<?php echo $department_code?>",
				url:"<?php echo $admin_url?>ad_data.php?call=update_team",
				beforeSend: function(){
					$(".response_gif").show();
				},
	...
	function loadData(){
		...
		requ = $.ajax({
			type:"POST",
			data:"bp_id=<?php echo $id?>&bp_sub_user_type=<?php echo $team_user_type?>&bp_id=<?php echo $id?>",
			dataType:"json",
			url:"<?php echo $admin_url?>ad_data.php?call=get_profile_team",
			beforeSend: function(){
				$(".response_gif").show();
			},
			success: function(data){
	...
	<?php
		if($id){}else{
	?>
	country_name(0, 'bcd_', 0, 0);
	country_name(0, 'bp_', 0, 0);
		<?php } ?>
