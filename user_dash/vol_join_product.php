...
loadData(page,fields,ord_sort);
	function loadData(page,fields,ord_sort){
		...
		var vol_id = "<?php echo $_SESSION['vol_db_id'];?>";
		//alert('vol id:- '+document.getElementById("vol_id").value);
		//alert(vol_id);
		...
		requ = $.ajax({
			type:"POST",
			data:"studnm="+studnm+"&scl_nm="+scl_nm+"&email="+email+"&vol_id="+vol_id+"&page="+page+"&fields="+fields+"&ord_sort="+ord_sort,
			url:"user_data.php?call=vol_join_stud_prod_list",
...
function emailid(){
	//alert(stat_no);
	var vl_id = document.getElementById("vol_id").value;
	$.ajax({
		type:"post",
		data:"vl_id="+vl_id, 
	...
	requ = $.ajax({
		...
		success:function(html){ 
			...
			if(html == 1){
				...
				window.location = "vol_join_product.php";
			}else if(html == 0) {
				alert("try again.");
			}else{
