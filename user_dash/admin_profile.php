...
$(document).ready(function() {
	...
	function loadData(){
		...
		requ = $.ajax({
			...
			dataType:"json",
			url:"<?php echo $admin_url?>ad_data.php?call=get_profile",
			beforeSend: function(){
				$(".response_gif").show();
			},
			success: function(data){
				...
				profile.forEach(function(elementObject){
					...
					keys.forEach(function(key){
					  ...
					  if(key == 'bp_crop_country' ){
							country_name(elementObject[key], 'bp_crop_', 0, 0);
							console.log('bp_crop_country');
					  }
					})
				});
					...
					keys.forEach(function(key){
						...
						if(key == 'boi_case'){
							if(elementObject[key] == 1){
								document.getElementById("ngo_case1").checked = true;
								document.getElementById("ngo_case2").checked = false;
								$("#boi_case_details").css({"display":"block"});
							} else {
								document.getElementById("ngo_case2").checked = true;
								document.getElementById("ngo_case1").checked = false;
								$("#boi_case_details").css({"display":"none"});
							}
