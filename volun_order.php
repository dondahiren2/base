...
<?php include("header.php");
	...
	if(isset($_SESSION['vol_db_id'])){
		$vol_id = $_SESSION['vol_db_id'];
	}else{
		$vol_id = "";
	}
?>
						...
						<section class="content">
							<div class="row"> 
								<div class="col-md-12">
									...
									<input type="hidden" id="vol_id" name="vol_id" value="<?php echo $vol_id; ?>" />
...
loadData(page,fields,ord_sort);    
	function loadData(page,fields,ord_sort){  
		...
		requ = $.ajax({
			type:"POST",
			data:"page="+page+"&vol_id="+vol_id+"&fields="+fields+"&ord_sort="+ord_sort,
			url:"user_data.php?call=vol_order",
			success: function(msg){
				//alert(msg);
				var msga = msg.split("pagination_part");
				//alert(msga[0]);
				$("#order_st_list").html(msga[0]);
