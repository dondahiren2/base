										...
										<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
											<thead>
												<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);">
													...
													<th style="width:10px">Order Tx Id</th>
													...
													<th style="width:100px;text-align:left">Delivery Note</th>
...
loadData(page,fields,ord_sort);    
	function loadData(page,fields,ord_sort){    
		...
		var vendor = "<?php echo $user_id; ?>";
		if(requ && requ.readyState != 4){
			requ.abort();
		}   
		requ = $.ajax({
			type:"POST",
			data:"page="+page+"&fields="+fields+"&ord_sort="+ord_sort+"&vendor="+vendor,
			url:"user_data.php?call=vendor_orders",
...
function delivery_not(id){
	window.open("order_note.php?id="+id);
}

function edit_order(id){
	var det = CKEDITOR.instances['ship_detils'].getData();
	//alert(det);
