...
<div class="page-bar">
	<ul class="page-breadcrumb">
		...
		<!-- <li>
			<a href="<?php //echo $admin_url?>registered_members">Student List</a>
			<i class="fa fa-angle-right"></i>
		</li> -->
		<li>
					...
					<table width="100%" class="table table-striped table-bordered table-vcenter" style="border:1px solid #fff;" cellspacing='0' cellpadding='5' >
						<thead>
							<tr style="background-color: rgb(46, 158, 177); color: rgb(255, 255, 255);width:500%;">
								...
								<th style="width:100px;text-align:left;">Student Name</th>
...
$(document).ready(function() {
	...
	function loadData(page,fields,ord_sort){
		...
		requ = $.ajax({
			...
			url:"<?php echo $admin_url?>ad_data.php?call=student_list",
	...
	function editbtn(id){ 
		window.location = "more_student.php?id="+btoa(id);
	}
	...
	function schl_name(){
		...
		$.ajax({
			...
			url:"<?php echo $admin_url?>ad_data.php?call=schl_nm",
			success:function(msg){
				//alert(msg);
				$("#sclnm").html(msg);
			}
	...
	function vol_name(){
		...
		$.ajax({
			type:"post",
			//data:"stat_no="+stat_no, 
			url:"<?php echo $admin_url?>ad_data.php?call=vol_nm",
			success:function(msg){
				//alert(msg);
				$("#vl_nm").html(msg);
			}
